...
# %%
import copy
def agg(self, by = ('sample', 'region', 'run', 'crystal_id'),
              how = 'mean',
              file_suffix: str = '_agg.h5', file_prefix: str = '', new_folder  = None,
              query = None, force_commensurate: bool = True) -> 'Dataset':
    ...
    for _, grp in gb:
        ...
        newshots['aggregation'] = newshots.iloc[0]['file'] + ':' + newshots.iloc[0]['Event'] + '->' +                                   newshots.iloc[-1]['file'] + ':' + newshots.iloc[-1]['Event']
...
#             print('Rechunking, this might be very inefficient. Please check the dask graph.')            
    ...
    return newset, initial_chunks, chunks
...
# %%
gb = ds.shots.groupby(['crystal_id'])
...
# %%
get_ipython().run_cell_magic('time', '', "rawagg = rawsel.aggregate(how='sum', query='frame < 5', \n                          force_commensurate=True)")
